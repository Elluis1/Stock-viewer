/** MD5 hex (para Gravatar). Implementación compacta sin dependencias. */
function md5Hex(input: string): string {
  ...
  function toHex(n: number): string {
    let out = '';
    for (let j = 0; j < 4; j++) {
      out += ((n >> (j * 8)) & 0xff).toString(16).padStart(2, '0');
    }
    return out;
  }
...
/**
 * URL de avatar: metadata OAuth (Google) > Gravatar del email > null (usar inicial).
 */
export function resolveAvatarUrl(opts: {
  ...
  const direct = opts.avatarUrl?.trim();
  if (direct) {
    return direct;
  }
...
export function avatarFromUserMetadata(meta: Record<string, unknown> | undefined | null): string | null {
  if (!meta) {
    return null;
  }
